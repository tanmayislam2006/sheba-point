...
async function refreshTokenMiddlewareForProxy(
  refreshToken: string,
): Promise<boolean> {
  try {
    ...
    console.error("Error refreshing token in middleware in proxy ts:", error);
    return false;
  }
}
...
export async function proxy(request: NextRequest) {
  try {
    ...
    const decodedAccessToken =
      accessToken &&
      jwtUtils.verifyToken(accessToken, env.JWT_ACCESS_SECRET as string).data;
    const isValidAccessToken =
      accessToken &&
      jwtUtils.verifyToken(accessToken, env.JWT_ACCESS_SECRET as string)
        .success;
    ...
    // Proactively refresh when access token is invalid/expired or about to expire.
    const shouldTryRefresh =
      !!refreshToken &&
      (!isValidAccessToken ||
        (accessToken ? await isTokenExpiringSoon(accessToken) : true));
    ...
    if (shouldTryRefresh) {
      ...
      try {
        const refreshed = await refreshTokenMiddlewareForProxy(refreshToken!);
        if (refreshed) {
...
// using the reverse proxy make the proxy matcher
export const config = {
  matcher: [
    /*
